...
def stmt(stream):
    if stream.pointer().type in ['VOID_TYPE']:
        ...
        return ('FUNDECL',
                ...
                args,
                body)
    # elif stream.pointer().type in ['TOINT']:
    #     stream.match('TOINT')
    #     e = exp(stream)
    #     return ('TOINT', e)
    # elif stream.pointer().type in ['TOFLOAT']:
    #     stream.match('TOFLOAT')
    #     e = exp(stream)
    #     return ('TOFLOAT', e)
    # elif stream.pointer().type in ['TOSTRING']:
...
def exp_high(stream):
    if stream.pointer().type in exp_lookahead:
        ...
        while stream.pointer().type in ['MUL','DIV']:
            ...
            e = (op_tk.type, e, tmp)
        return e
    elif stream.pointer().type in ['TOINT','TOFLOAT','TOSTRING']:
        if stream.pointer().type == 'TOINT':
            op_tk = stream.match('TOINT')
        elif stream.pointer().type == 'TOFLOAT':
            op_tk = stream.match('TOFLOAT')
        else:
            op_tk = stream.match('TOSTRING')
        e = exp(stream)
        return (op_tk.type, e)
    else:
